...
describe('Компонент CardForm', () => {
  describe('Рендер', () => {
    ...
    describe('Должен присутствовать элемент с классом ', () => {
      ...
      it('.left-time', () => {
        expect(wrapper.find('.left-time')).toHaveLength(1);
      });
    ...
    describe('Методы класса', () => {
      ...
      describe('componentDidMount', () => {
        ...
        it('При монтировании запускает интервал', () => {
          jest.useFakeTimers();
          const wrapper = mount(<CardForm onChangeTimeOver={jest.fn()} />);
          wrapper.update();
          expect(setInterval.mock.calls.length).toBe(1);
        });
        it('Интервал меняет состояние компоненты state.leftTime', () => {});
      });
